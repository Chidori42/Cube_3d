...
/*   Updated: 2024/11/03 10:29:30 by yakazdao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	horz_intersection(t_data *dt, float angle)
{
	...
	{
		1 && (dt->x_check = dt->x_intr, dt->y_check = dt->y_intr);
		if (dt->ray->ray_facing_up)
			dt->y_check -= 1;
	...
	1 && (dt->ray->h_wall_x_hit = dt->x_intr, \
		dt->ray->h_wall_y_hit = dt->y_intr);
	return (sqrt(pow(dt->player->x - dt->ray->h_wall_x_hit, 2) \
		+ pow(dt->player->y - dt->ray->h_wall_y_hit, 2)));
}
...
float	vert_intersection(t_data *dt, float angle)
{
	...
	1 && (dt->ray->v_wall_x_hit = dt->x_intr, \
		dt->ray->v_wall_y_hit = dt->y_intr);
	return (sqrt(pow(dt->player->x - dt->ray->v_wall_x_hit, 2) + \
		pow(dt->player->y - dt->ray->v_wall_y_hit, 2)));
}
